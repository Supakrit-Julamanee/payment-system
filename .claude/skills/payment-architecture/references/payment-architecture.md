...
**ข้อ 2 ตัดออกจากการทดสอบ (ตัดสินใจ 2026-09-16):** บัญชีทดสอบของ Omise ไม่ได้เปิด 3DS เป็นค่าเริ่มต้น ต้องขอ `support@omise.co` ให้เปิดก่อน (หลักฐานในตารางท้ายหัวข้อ 18) โปรเจกต์นี้เลือกไม่ขอเปิด จึงไม่ทดสอบหน้ายืนยันตัวตนจริง แต่**โค้ดที่รองรับ 3DS ยังอยู่ครบ** ได้แก่ การส่ง `return_uri` (หัวข้อ 13.2) และการ redirect ไป `authorize_uri` เมื่อ `status = pending` (หัวข้อ 12.3) ซึ่งมี test แบบ mock คลุมอยู่ ถ้าวันหลังเปิด 3DS ให้บัญชีแล้ว ทดสอบข้อนี้ได้ทันทีโดยไม่ต้องแก้โค้ด
...
| 3DS ใน test mode | บัตร 3DS (เช่น Visa `4111 1111 1115 0002` = enrollment ล้มเหลว, `4111 1111 1114 0003` = validation ล้มเหลว) **ใช้ได้เฉพาะบัญชีที่เปิด 3DS แล้ว** และ charge ต้องส่ง `return_uri` ไปด้วย (เราส่งอยู่แล้วตามหัวข้อ 13.2) ข้อความจริงในเอกสาร: "3DS testing requires a 3DS-enabled account. Contact support@omise.co to enable 3DS for your test account before proceeding with 3DS testing scenarios." / "When enabling 3-D Secure on your account, the service will first be enabled on test mode, then - once implemented - we will enable the service on your live account." / "Our fraud analysts will determine whether enabling the service on your account is necessary based on your business's risk profile." สรุป: 3DS เป็นบริการระดับบัญชีที่ Omise เป็นผู้เปิด ไม่ได้เปิดเป็นค่าเริ่มต้น | https://docs.omise.co/api-testing/thailand, https://docs.omise.co/how-to-implement-3-D-Secure, https://docs.omise.co/3d-secure |
| 3DS ในบัญชีนี้ (ตรวจจริง) | 2026-09-16: charge บัตร `4242...` ทั้งสองใบมี `authorize_uri` ลงท้าย `?acs=false` และ `status = successful` ตั้งแต่แรก เปิด `authorize_uri` ได้ 302 ไป `/payments/.../complete` ซึ่งเป็นหน้า "Processing..." ที่ฝัง `return_uri` ของเราไว้พากลับทันที ไม่มีหน้ายืนยันตัวตนหรือ OTP คั่น ยืนยันว่าบัญชีทดสอบนี้ยังไม่ได้เปิด 3DS | ทดสอบจริง |
